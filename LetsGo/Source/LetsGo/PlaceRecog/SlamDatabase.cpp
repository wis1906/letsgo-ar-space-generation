...
void SlamDatabase::InitDataForXYZ()
{
	...
	while (!dataFile.eof())
	{
		...
		if (line != "")
		{
			...
			//convert coordinate
			FVector posVec(landmark->pos_w.x, landmark->pos_w.y, landmark->pos_w.z);
			landmark->posConv = convertCoord(posVec);
...
void SlamDatabase::InitData()
{
	...
	for (int i = 0; i < numKeyframeCamera; i++)
	{
		...
		//convert coordinate
		FVector camTransVec(cam->camTrans.x, cam->camTrans.y, cam->camTrans.z);
		cam->camTransConv = convertCoord(camTransVec);
		FRotator rotVec; rotVec.Roll = cam->camRotEuler.x; rotVec.Pitch = cam->camRotEuler.y; rotVec.Yaw = cam->camRotEuler.z;
		cam->camRotEulerConv = convertCoord(rotVec);
	...
	for (int i = 0; i < numFrameCamera; i++)
	{
		...
		for (int row = 0; row < 3; row++)
		{
			for (int col = 0; col < 4; col++)
			{
				if (isnan(cam->camMat.at<double>(row, col)))
					cam->camRot.at<double>(row, col) = 0;
				else
					cam->camRot.at<double>(row, col) = cam->camMat.at<double>(row, col);
		...
		if (isnan(cam->camMat.at<double>(0, 3)))
			cam->camTrans.x = 0;
		else
			cam->camTrans.x = cam->camMat.at<double>(0, 3) * mapScale;

		if (isnan(cam->camMat.at<double>(1, 3)))
			cam->camTrans.y = 0;
		else
			cam->camTrans.y = cam->camMat.at<double>(1, 3) * mapScale;

		if (isnan(cam->camMat.at<double>(2, 3)))
			cam->camTrans.z = 0;
		else
			cam->camTrans.z = cam->camMat.at<double>(2, 3) * mapScale;
...
//##########################
void SlamDatabase::initCurFrameData(int frameNum)
{
	curFrameNum = frameNum;

	//keyframe camera
	curFrameKFs.clear();
	for (int i = 0; i < keyframeCamDataList.size(); i++)
	{
		if (keyframeCamDataList[i]->id_frame > frameNum)
			continue;

		curFrameKFs.push_back(keyframeCamDataList[i]);
	}

	//frame camera
	curFrameCams.clear();
	for (int i = 0; i < frameCamDataList.size(); i++)
	{
		if (frameCamDataList[i]->id_frame > frameNum)
			continue;

		curFrameCams.push_back(frameCamDataList[i]);
	}

	//landmark
	curFrameLandmarks.clear();
	for (int i = 0; i < landmarkDataList.size(); i++)
	{
		if (landmarkDataList[i]->id_frame > frameNum)
			continue;

		curFrameLandmarks.push_back(landmarkDataList[i]);

	}

	//landmark inliers
	curFrameInlierLandmarks.clear();
	for (int i = 0; i < curFrameLandmarks.size(); i++)
	{
		if (curFrameLandmarks[i]->isOutlier == true)
		{
			continue;
		}
		curFrameInlierLandmarks.push_back(curFrameLandmarks[i]->posConv);
	}
}
